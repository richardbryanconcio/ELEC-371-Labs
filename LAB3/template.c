...
#define TIMER1_SNAP_LO	((volatile unsigned int *) 0x10004034)

#define TIMER1_SNAP_HI	((volatile unsigned int *) 0x10004038)
...
void interrupt_handler(void)
{
   ...
   // TIMER3 10000000000000000
   if (ipending & 0x10000) {
      *TIMER3_STATUS = 0;
	  *JTAG = '\b';
	  if (*SWITCHES & 0x8) {		// 0000001000
		if (iteration1 % 2 == 0) {
			*JTAG = '!';
		} else {
			*JTAG = '?';
   ...
   // TIMER2 01000000000000000
   if (ipending & 0x8000) {
      *TIMER2_STATUS = 0;
	  if (iteration2 % 4 == 0) {	// 00000001 00000001 00000001
		*HEX_DISPLAY = 0x1010101;
	  }
	  if (iteration2 % 4 == 1) {	// 01000000 01000000 01000000
		*HEX_DISPLAY = 0x40404040;
	  }
	  if (iteration2 % 4 == 2) {	// 00001000 00001000 00001000
		*HEX_DISPLAY = 0x8080808;
	  }
	  if (iteration2 % 4 == 3) {	// 01000000 01000000 01000000
		*HEX_DISPLAY = 0x40404040;
	  }
	  iteration2++;
   }
   // TIMER1 00100000000000000
   if (ipending & 0x4000) {
      *TIMER1_STATUS = 0;	
	  if (iteration3 % 5 == 0) {	// 0000000011
		*LEDS = 0x3;
	  }
	  if (iteration3 % 5 == 1) {	// 0000001100
		*LEDS = 0xC;
	  }
	  if (iteration3 % 5 == 2) {	// 0000110000
		*LEDS = 0x30;
	  }
	  if (iteration3 % 5 == 3) {	// 0011000000
		*LEDS = 0xC0;
	  }
	  if (iteration3 % 5 == 4) {	// 1100000000
		*LEDS = 0x300;
	  }
	  iteration3++;
   }
...
void Init (void)
{
	...
	// Set the timer3 to generate interrupts every 0.5 seconds
	*TIMER3_START_HI = 25000000 >> 16;
	*TIMER3_START_LO = 25000000 & 0xFFFF;
	*TIMER3_CONTROL = 0x7;
	
	// Set the timer2 to generate interrupts every 0.25 seconds
	*TIMER2_START_HI = 12500000 >> 16;
	*TIMER2_START_LO = 12500000 & 0xFFFF;
	*TIMER2_CONTROL = 0x7;
	
	// Set the timer2 to generate interrupts every 0.2 seconds
	*TIMER1_START_HI = 10000000 >> 16;
	*TIMER1_START_LO = 10000000 & 0xFFFF;
	*TIMER1_CONTROL = 0x7;
	
	
	/* set up ienable */
	NIOS2_WRITE_IENABLE(0x1C000);  // enable interrupts for timer at bit 0 and pushbuttons at bit 1
