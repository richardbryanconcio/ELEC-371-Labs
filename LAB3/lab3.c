...
// pointer macros for push button registers
#define PUSH_BUTTON_DATA   ((volatile unsigned int *) 0x10000050)

#define PUSH_BUTTON_MASK	((volatile unsigned int *) 0x10000058)

#define PUSH_BUTTON_EDGE	((volatile unsigned int *) 0x1000005C)

// pointer macros for 7-Segment Displays Parallel Port
#define HEX_DISPLAY        ((volatile unsigned int *) 0x10000020)


/* define global program variables here */
unsigned int g_timer_overflows; // to count timer overflows
...
void interrupt_handler(void)
{
   ...
   // CHECK FOR PUSH BUTTONS INTERRUPT
   if (ipending & 0x2) {
      handle_push_buttons();
   }
...
void Init (void)
{
   ...
   // Set the timer to generate interrupts every 0.5 seconds
   *TIMER_START_HI = 0x02FA;
   *TIMER_START_LO = 0x0F080;
   *TIMER_CONTROL = 0x7;

   // Set up pushbutton interrupts
   *PUSH_BUTTON_MASK = 0x3;


	/* set up ienable */
   NIOS2_WRITE_IENABLE(0x3);  // enable interrupts for timer at bit 0 and pushbuttons at bit 1
